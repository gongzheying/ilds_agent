CREATE TABLE if not exists `tbl_ilds_transfer_package`
(
...
CREATE TABLE if not exists `tbl_ilds_transfer_file`
(
    `id`                  bigint(20) NOT NULL AUTO_INCREMENT,
    `created_at`          datetime     DEFAULT NULL,
    `last_modified_at`    datetime     DEFAULT NULL,
...
CREATE TABLE if not exists `tbl_ilds_transfer_site`
(
    `id`                   bigint(20) NOT NULL AUTO_INCREMENT,
    `created_at`           datetime            DEFAULT NULL,
    `last_modified_at`     datetime            DEFAULT NULL,
    ...
    KEY `tbl_ilds_transfer_site_FK` (`credential_id`),
    CONSTRAINT `tbl_ilds_transfer_site_FK` FOREIGN KEY (`credential_id`) REFERENCES `tbl_ilds_transfer_credentials` (`id`)
) ENGINE = InnoDB;


CREATE TABLE if not exists `tbl_ilds_pgp_encryption_key`
(
    `id`               bigint(20) NOT NULL AUTO_INCREMENT,
    `created_at`       datetime     DEFAULT NULL,
    `last_modified_at` datetime     DEFAULT NULL,
